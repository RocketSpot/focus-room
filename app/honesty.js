...
const RULES = [
  ...
  { name: 'percentage-score',
    ...
    allow: (m) => {
      ...
      // score phrasing ("score rose 25") is caught by the numeric-score rule.
      const head = m.input.slice(Math.max(0, m.index - 12), m.index).toLowerCase();
      return /\b(?:rose|fell)\s+$/.test(head);
    } },
...
function redact(text) {
  ...
  for (const r of RULES) {
    ...
    s = s.replace(r.re, (...args) => {
      ...
      if (r.allow) {
        const m = [match];
        // rebuild the capture groups replace() handed us, so allow() sees the
        // same shape exec() would have produced
        for (let i = 1; i < args.length - 2; i++) m.push(args[i]);
        m.index = index; m.input = input;
        if (r.allow(m)) return match;
      }
      return '';
    });
  }
