...
class Orchestrator extends EventEmitter {
  ...
  onClientMessage(msg, role) {
    this._touchWatchdog(); // any client message counts as activity
    // Only the guest's iPad drives the FSM — any other LAN client (a stray
    // laptop on the room network, a TV surface, a probe) is ignored.
    if (role !== 'ipad') return;
    // a real guest tapping through takes over from the sim demo autopilot
    if (this._demoActive && !this._demoSending) { this._demoStop(); this.log('demo: real guest input — autopilot yielding'); }
  ...
  _uiCue(msg) {
    const accepted =
      ...
      (msg.type === CLIENT.GUEST_EVENT && ({
        ...
        fit: [GUEST_EVENT.BASELINE_START, 'fit_confirmed'],  // "I'm ready" is a tap too
        standby: ['reveal_ack'],
  ...
  _onGuestEvent(msg) {
    ...
    switch (this.beat) {
      ...
      case 'strongest':
        ...
        break;
      case 'standby':
        if (kind === 'reveal_ack') this._finishReveal();
        break;
      case 'email':
      ...
      case 'close':
        if (kind === GUEST_EVENT.CLOSE_CHOICE) {
          // the matched door is known now → send the report to the captured email
          this.answers.closeDoor = (msg.payload && msg.payload.door) || 'investor';
  ...
  _beginReveal() {
    ...
    this.log(`reveal: ${this.reveal.archetype.name}${this.reveal.flat ? ' (flat session)' : ''}${this.signalIssue ? ' (signal issue)' : ''}`);
  ...
  _fireInterruption() {
    ...
    // fallback try again.
    if (this._eegDown) {
      this.log('interruption due, but the EEG stream is down — deferring until the signal returns');
      this._clearReadingFallback();
      this._readingFallback = setTimeout(() => {
