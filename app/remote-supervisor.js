...
class RemoteSupervisor extends EventEmitter {
    ...
    server.on('client-message', ({ id, role, msg }) => {
      ...
      if (msg.type === 'bridge/hello') {
        if (msg.token !== this.token) {
          console.error('[bridge] REJECTED a bridge with a wrong token');
          return;
        }
        this.bridgeId = id;
        // the desktop sidecar usually booted before the link opened — its
        // mode (sim vs real buds) rides the bridge hello so the badge is
        // honest from the first second
        if (msg.info && this.lastHello == null) {
          this.lastHello = { simulate: !!msg.info.simulate, source: msg.info.source || null };
        }
        console.log('[bridge] desktop bridge connected');
        this.server.broadcast('bridge/accepted', {}, 'bridge');
        this.emit('connected');
        return;
      }
  ...
  send(type, payload = {}) {
    ...
    try { this.server.broadcast('bridge/cmd', { cmd: type, payload }, 'bridge'); return true; }
