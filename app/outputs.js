...
function buildData(reveal, answers, dateStr) {
  ...
  const caption = st
    ? [
      ...
      st.realDip ? `The notification cost ${secWord(st.recoverSec)}.` : 'The notification cost nothing measurable.',
  ...
  const data = {
    ...
    troughT: reads[2] ? reads[2].anchorT : 0.6,
    // card: 3 short lines
    caption: caption.map((line, i) => honest(line, `card.cap${i + 1}`)),
    // profile: one descriptive line, no scores
    desc: honest(`${cap(reads[0] ? reads[0].v : 'a calm settle')}, then ${reads[1] ? reads[1].v : 'even focus'}.`, 'profile.desc'),
