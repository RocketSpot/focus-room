...
function computeReads({ samples, answers, interruptEegT, signalIssue, bands }) {
  ...
  const interruptT = interruptEegT != null ? clamp01(interruptEegT / totalT) : 0.6;
  ...
  const rawDip = Math.max(0, vrAt(Math.max(0, interruptT - 0.02)) - troughVr);
  ...
  const mids = fin.filter((p) => {
    const f = frac(p.t);
    return f >= engagedFrom && f <= 0.9 && !(f >= interruptT - 0.02 && f <= recoveredT);
  });
  ...
  for (let s = 0; s <= 1 - WIN; s += 0.01) {
    if (s < recoveredT && s + WIN > interruptT) continue;
    let sum = 0, c = 0;
  ...
  const settlePctShown = totalT > 0 ? Math.max(1, Math.round((round5(settleSec) / Math.round(totalT)) * 100)) : 0;
  ...
  // we say so.
  const realDip = dipPctOwn >= 5 && rawDip >= 0.04;
  ...
  const intShift = interruptBandShift(bands, interruptT);
  ...
  const reads = [
    ...
    },
    {
      ...
      anchorT: troughT,
      k: 'Interruption', v: realDip ? dipWord : (intVisible ? 'a shift in the bands' : 'a small ripple'), color: 'orange',
  ...
  return {
    ...
    interruptT, samplesForReveal: fin,
...
// couldn't compute must not appear at all.
function minimalReads() {
  return [
    { index: 1, no: '01', title: 'How you settled', type: 'span', r0: 0, r1: 0.4, anchorT: 0.2, k: 'Settle', v: 'a calm settle', color: 'signal', sentence: 'Your read started cleanly and held.' },
    { index: 2, no: '02', title: 'Your attention rhythm', type: 'span', r0: 0.3, r1: 0.6, anchorT: 0.45, k: 'Rhythm', v: 'even and level', color: 'signal', sentence: 'Your focus ran steady through the clean stretch of your session.' },
    ...
    { index: 4, no: '04', title: 'Your strongest stretch', type: 'span', r0: 0.7, r1: 0.85, anchorT: 0.78, k: 'Strongest', v: 'your best run today', color: 'signal', sentence: 'This stretch ran highest, relative to the rest of your reading.' },
  ];
}
