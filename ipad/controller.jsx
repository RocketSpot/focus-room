...
(function () {
  ...
  const GO_EVENT = { welcome: 'earbud_seated', fit: 'fit_confirmed', reading: 'reading_finished',
  ...
  function boot() {
    ...
    // Real iPad app: launched from the home screen (navigator.standalone / the
    // standalone display-mode), or forced with ?kiosk=1. Drops the preview bezel
    // and fills the screen. A plain desktop browser stays in framed preview.
    const standalone = window.navigator.standalone === true ||
      (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches);
    const deviceMode = standalone || params.get('kiosk') === '1';
    if (deviceMode) document.body.classList.add('device');

    function fit() {
      const room = document.getElementById('room'); if (!room) return;
      const s = deviceMode
        ? Math.max(window.innerWidth / 816, window.innerHeight / 1172)   // fill the screen (cover)
        : Math.min(window.innerWidth / 920, window.innerHeight / 1276);  // framed desktop preview
