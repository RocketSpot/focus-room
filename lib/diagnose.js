...
(function (root) {
  ...
  function diagnose(s) {
    ...
    if (streaming && s.lastFrameAgo != null && s.lastFrameAgo > 6000 && s.analysisAlive) {
      add('info', 'analysis-picky', 'Movement is pausing the analysis, the link is fine',
        'The earbuds are connected and streaming; the analyser is rejecting windows (movement or settling electrodes) and the line resumes on the next clean stretch.',
        ['Nothing to reconnect: the Bluetooth link is healthy.',
         'If it lasts, ask the guest to settle and keep still for a few seconds.']);
    } else if (streaming && s.lastFrameAgo != null && s.lastFrameAgo > 6000) {
